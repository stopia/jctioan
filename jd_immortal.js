...
async function main() {
  ...
  if (!$.body) {
    await new Promise(async (resolve) => {
      $.http.get({url: `https://purge.jsdelivr.net/gh/Tersd07/st1@master/jd_immortal.js`}).then((resp) => {
  ...
  if ($.body) {
    $.body = $.body.replace(
      ...
      "$1'23xIs4YwE5Z7HnHnaabzTbKvtIVp1Ygwc','23xIs4YwE5Z7HnHnaabzTbKvtIVp1Ygwc'"
    ).replace(
...
function updateShareCodes(url = `https://raw.githubusercontent.com/Tersd07/st1/master/jd_immortal.js`) {
...
function updateShareCodesCDN(url = `https://cdn.jsdelivr.net/gh/Tersd07/st1@master/jd_immortal.js`) {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
